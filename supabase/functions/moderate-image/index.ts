...
Deno.serve(async (req) => {
  try {
    ...
    if (!approved) {
      ...
      return new Response(JSON.stringify({ approved: false, reason }), { headers: { "Content-Type": "application/json" } });
    }

    return new Response(JSON.stringify({ approved: true }), { headers: { "Content-Type": "application/json" } });

  } catch (err) {
    return new Response(JSON.stringify({ approved: true, warning: err.message }), { headers: { "Content-Type": "application/json" } });
  }
});
